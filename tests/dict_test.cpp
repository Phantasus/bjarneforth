...
void
test_executing_nested_dolink_word ()
{
  bf_state state;
  cell xt1;
  cell xt2;
  ...
  bf_define_literal (&state, "four", 4);
  bf_define_literal (&state, "five", 5);

  xt1 = bf_lookup_word_xt (&state, "combined");
  xt2 = bf_lookup_word_xt (&state, "four");
  xt3 = bf_lookup_word_xt (&state, "five");

  bf_define_word (&state, "combined2", normal_word, &bf_prim_dolink, xt1);

  bf_inlinecell (&state, xt2);
  bf_inlinecell (&state, xt3);
  bf_inlineuint (&state, 0); /* exit */

  char word_label[] = "combined2";
  bf_push_dstack_char_ptr (&state, word_label);
  bf_push_dstack_int (&state, sizeof (word_label) - 1);
  bf_prim_lookup (&state);
  ...
  ASSERT_EQUAL (bf_size_dstack (&state), 5,"Should be three items on the dstack");
  ASSERT_EQUAL (bf_pop_dstack_int (&state), 5, "Should be three");
  ASSERT_EQUAL (bf_pop_dstack_int (&state), 4, "Should be three");
  ASSERT_EQUAL (bf_pop_dstack_int (&state), 3, "Should be three");
  ASSERT_EQUAL (bf_pop_dstack_int (&state), 2, "Should be two");
  ASSERT_EQUAL (bf_pop_dstack_int (&state), 1, "Should be one");
