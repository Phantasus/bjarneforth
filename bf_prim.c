...
void
prim_towordtoken (bf_state *state)	/* ( xt -- wt ) */
{
...
void
prim_repeat (bf_state *state)	/* ( -- adr ) */
{
  if (state->vars.state)
    bf_push (&(state->dstack), (cell) state->vars.here);
}
